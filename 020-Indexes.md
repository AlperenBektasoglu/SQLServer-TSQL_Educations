...
Sql Server’da heap table adında somut bir kavram yoktur. Bir tabloya heap denilmesi aslında onun üzerinde bir index tanımlı olup olmamasına bağlıdır. Sql Server bir veriyi indexsiz bir tabloya eklerken sıralı olarak diskte tutmaz ve veriler rastgele data page’lere yazılır. Bu şekilde olan tablolar heap table diye adlandırılır. Yani üzerinde clustered index olmayan tablolar heap table’dır diyebiliriz.
...
- Bir tabloda clustured indeks olan kolonun dışında farklı bir kolon üzerinden tekrar indeksleme yapılmak isteniyorsa non-clustured indeks kullanılır ve bunun için tekrar bir dengeli ağaç yapısı(balance tree) oluşturulur.
...
Önce nonclustured index'in B-tree yapısından tek bir dolanma ile bütün verileri bulur. Ardından her veri içintek tek clustered indexte yada heapte arama yapar.
...
BEGIN
	INSERT Deneme_1
	SELECT @sayac, 'Alperen' + CAST(@sayac AS varchar(10)), 'Bektaşoğlu' + CAST(@sayac AS varchar(10)), '555', 'Meslek' + CAST(@sayac AS varchar(10))
	SET @sayac = @sayac + 1
END
